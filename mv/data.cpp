...
void fill_rand_sparse_pattern(float* data, const int size, const float sparse_rate) {
    ...
    const int gran = 4;
    int zero_count = 0;
    int zero_num = ((int)(size*sparse_rate/4))*4;
    for(i = 0; i< (size/4); ++i) {
        float indicate = (float)rand() / INT_MAX;
        if (((indicate>sparse_rate)&((zero_num-zero_count)<=(size-4*i-1)))|(zero_count == zero_num)) {
            data[4*i+0] = (float)rand() / INT_MAX;
            data[4*i+1] = (float)rand() / INT_MAX;
            data[4*i+2] = (float)rand() / INT_MAX;
            data[4*i+3] = (float)rand() / INT_MAX;
        }
        else {
            data[4*i+0] = 0.0;
            data[4*i+1] = 0.0;
...
void print_data(float* data, const int size) {
